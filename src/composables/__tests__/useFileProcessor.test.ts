...
describe("useFileProcessor", () => {
   ...
   */
  function setupFullProcessingMocks(options: {
    pageCount?: number;
    ocrText?: string;
  } = {}) {
    ...
    vi.mocked(invoke).mockImplementation(async (cmd: string) => {
      switch (cmd) {
        ...
        case "split_pdf":
          return {
            imagePaths: Array.from({ length: pageCount }, (_, i) => `/tmp/page_${i + 1}.png`),
            tempDir: "/tmp/split",
          };
  ...
  describe("collectFiles", () => {
    ...
    it("ignores files without extensions", async () => {
      vi.mocked(readDir).mockResolvedValue([
        ...
        { name: "Makefile", isFile: true, isDirectory: false, isSymlink: false },
      ]);
  ...
  describe("selectFile", () => {
    ...
    it("processes file through full pipeline when authenticated", async () => {
      ...
      expect(invoke).toHaveBeenCalledWith("upload_to_google_drive", expect.any(Object));
  ...
  describe("processFiles - image processing", () => {
    ...
    it("processes single image file directly (no PDF split)", async () => {
      ...
      expect(invoke).not.toHaveBeenCalledWith("get_pdf_page_count", expect.any(Object));
  ...
  describe("processFiles - PDF processing", () => {
    ...
    it("splits PDF into pages before OCR", async () => {
      ...
      expect(invoke).toHaveBeenCalledWith("split_pdf", expect.objectContaining({
        pdfPath: "/path/to/document.pdf",
        totalPages: 3,
      }));
    });
    ...
    it("processes each page through OCR", async () => {
      ...
      // 3 pages = 3 uploads
      const uploadCalls = vi.mocked(invoke).mock.calls.filter(
        call => call[0] === "upload_to_google_drive"
      );
