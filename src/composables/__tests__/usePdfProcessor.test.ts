...
describe("usePdfProcessor", () => {
  ...
  describe("cleanupTempDir", () => {
    ...
    it("propagates errors from invoke", async () => {
      ...
      await expect(cleanupTempDir("/tmp/test")).rejects.toThrow("Cleanup failed");
    });
  });
  ...
  describe("splitPdf", () => {
    ...
    it("sets up progress listener when callback provided", async () => {
      ...
      expect(listen).toHaveBeenCalledWith("split-progress", expect.any(Function));
    ...
    it("cleans up listener even on error", async () => {
      ...
      await expect(splitPdf("/path/to/file.pdf", 150, () => {})).rejects.toThrow(
        "Split failed"
      );
  ...
  describe("extractPage", () => {
    it("extracts single page from PDF", async () => {
      ...
      const result = await extractPage("/path/to/file.pdf", 1, 150, "/output/page.png");
