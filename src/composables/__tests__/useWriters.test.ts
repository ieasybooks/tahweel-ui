...
describe("useWriters", () => {
  const { isArabicText, compactText, writeTxt, writeJson, writeOutputs } = useWriters();
  ...
  describe("compactText", () => {
    ...
    it("merges shortest adjacent lines when over 40 lines", () => {
      const lines = Array(50).fill("A").map((_, i) => `Line ${i}`);
  ...
  describe("writeTxt", () => {
    ...
    it("uses custom page separator", async () => {
      ...
      expect(writeTextFile).toHaveBeenCalledWith("/output/test.txt", "Page 1\n---\nPage 2");
    });
  ...
  describe("writeJson", () => {
    ...
    it("writes JSON with page numbers", async () => {
      ...
      expect(writeTextFile).toHaveBeenCalledWith("/output/test.json", expectedJson);
    });
